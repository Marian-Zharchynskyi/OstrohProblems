...
export function AiChat() {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const response = await chatService.sendMessage({ message: inputValue.trim() }, token ?? null);
  ...
  const renderProblemCard = (problem: ProblemSummaryChat) => (
    <div key={problem.id} onClick={() => navigateToProblem(problem.id)} className="bg-white border border-gray-200 rounded-lg p-3 cursor-pointer hover:shadow-md transition-shadow">
  ...
  const renderMessage = (message: ChatMessage) => {
    ...
    return (
      <div key={message.id} className={cn('flex gap-2 mb-4', isUser ? 'flex-row-reverse' : 'flex-row')}>
        ...
        <div className={cn('max-w-[80%] rounded-2xl px-4 py-2', isUser ? 'bg-violet-500 text-white rounded-tr-sm' : 'bg-gray-100 text-gray-900 rounded-tl-sm')}>
          {isUser ? (
            <div className="text-sm whitespace-pre-wrap">{message.content}</div>
          ) : (
            <div className="text-sm prose prose-sm max-w-none whitespace-pre-wrap prose-headings:mt-3 prose-headings:mb-2 prose-p:my-3 prose-p:leading-relaxed prose-ul:my-2 prose-ol:my-2 prose-li:my-1">
              <ReactMarkdown remarkPlugins={[remarkGfm]}>{message.content}</ReactMarkdown>
            </div>
          )}
          {message.problems && message.problems.length > 0 && <div className="mt-3 space-y-2">{message.problems.map(renderProblemCard)}</div>}
  ...
  return (
    <>
      ...
      <button
        ...
        {isOpen ? <X className="w-6 h-6 text-gray-700" /> : <MessageCircle className="w-6 h-6 text-white" />}
      </button>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={cn(
                ...
                isRecording ? 'bg-red-500 text-white hover:bg-red-600' : 'bg-violet-100 text-violet-600 hover:bg-violet-200'
              )}
