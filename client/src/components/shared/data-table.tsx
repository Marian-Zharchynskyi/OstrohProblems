...
export function DataTable<T extends { id: string | null }>({ data, columns, onEdit, onDelete, onRowClick, isLoading, emptyMessage = 'No data available' }: DataTableProps<T>) {
  if (isLoading) {
    return (
  ...
  return (
    <div className="rounded-md border">
      <Table>
        ...
        <TableBody>
          {data.map((item) => (
            <TableRow key={item.id || Math.random()} onClick={() => onRowClick?.(item)} className={onRowClick ? 'cursor-pointer hover:bg-muted/50' : ''}>
              {columns.map((column, colIndex) => {
                const value = getCellValue(item, column);
                return <TableCell key={colIndex}>{column.cell ? column.cell(value, item) : String(value ?? '')}</TableCell>;
              })}
              {(onEdit || onDelete) && (
