...
export const problemsApi = {
  ...
  getPaged: async (page: number = 1, pageSize: number = 10) => {
    const response = await apiClient.get<PagedResult<Problem>>(`${BASE_URL}/paged`, { page, pageSize });
    return response.data;
  },
  ...
  uploadImages: async (id: string, files: FileList) => {
    ...
    const response = await apiClient.putFormData<Problem>(`${BASE_URL}/upload-images/${id}`, formData);
    return response.data;
  },

  deleteImage: async (problemId: string, imageId: string) => {
    const response = await apiClient.put<Problem>(`${BASE_URL}/delete-image/${problemId}`, { problemImageId: imageId });
    return response.data;
  },

  assignCoordinator: async (problemId: string, coordinatorId: string, priority?: string) => {
    const response = await apiClient.put<Problem>(`${BASE_URL}/assign-coordinator/${problemId}`, { coordinatorId, priority });
    return response.data;
  },

  reject: async (problemId: string, coordinatorId: string, rejectionReason: string) => {
    const response = await apiClient.put<Problem>(`${BASE_URL}/reject/${problemId}`, { coordinatorId, rejectionReason });
    return response.data;
  },

  setCoordinatorComment: async (problemId: string, comment: string) => {
    const response = await apiClient.put<Problem>(`${BASE_URL}/set-coordinator-comment/${problemId}`, comment);
    return response.data;
  },

  updateCurrentState: async (problemId: string, currentState: string) => {
    const response = await apiClient.put<Problem>(`${BASE_URL}/update-current-state/${problemId}`, { currentState });
    return response.data;
  },
  completeProblem: async (problemId: string, currentState: string) => {
    const response = await apiClient.put<Problem>(`${BASE_URL}/complete/${problemId}`, currentState);
    return response.data;
  },
  ...
  uploadCoordinatorImages: async (id: string, files: FileList) => {
    ...
    const response = await apiClient.putFormData<Problem>(`${BASE_URL}/upload-coordinator-images/${id}`, formData);
    return response.data;
  },

  deleteCoordinatorImage: async (problemId: string, imageId: string) => {
    const response = await apiClient.put<Problem>(`${BASE_URL}/delete-coordinator-image/${problemId}`, { coordinatorImageId: imageId });
    return response.data;
  },

  updateDescription: async (problemId: string, description: string) => {
    const response = await apiClient.put<Problem>(`${BASE_URL}/update-description/${problemId}`, { description });
    return response.data;
  },

  updateTitleAndCategories: async (problemId: string, title: string, categoryNames?: string[]) => {
    const response = await apiClient.put<Problem>(`${BASE_URL}/update-title-and-categories/${problemId}`, { title, categoryNames });
    return response.data;
  },

  updateLocation: async (problemId: string, latitude: number, longitude: number) => {
    const response = await apiClient.put<Problem>(`${BASE_URL}/update-location/${problemId}`, { latitude, longitude });
    return response.data;
  },
};
