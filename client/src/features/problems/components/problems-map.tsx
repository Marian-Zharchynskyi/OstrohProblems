...
export function ProblemsMap({ problems }: ProblemsMapProps) {
  ...
  return (
    <>
            ...
            <Marker
              ...
              <Popup>
                <div className="min-w-[200px]">
                  ...
                  {problem.status && <span className="inline-flex items-center rounded-full bg-blue-50 px-2 py-1 text-xs text-blue-700">{problem.status}</span>}
                  <button
      ...
      {selectedProblem && <ProblemDetailPopup problem={selectedProblem} onClose={() => setSelectedProblem(null)} />}
    </>
  );
}
