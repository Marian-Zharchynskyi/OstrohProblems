...
export function useUpdateProblem() {
  ...
  return useMutation({
    mutationFn: ({ id, data }: { id: string; data: CreateProblem }) => problemsApi.update(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [PROBLEMS_QUERY_KEY] });
    },
  });
...
export function useUploadProblemImages() {
  ...
  return useMutation({
    mutationFn: ({ id, files }: { id: string; files: FileList }) => problemsApi.uploadImages(id, files),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [PROBLEMS_QUERY_KEY] });
    },
  });
...
export function useDeleteProblemImage() {
  ...
  return useMutation({
    mutationFn: ({ problemId, imageId }: { problemId: string; imageId: string }) => problemsApi.deleteImage(problemId, imageId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [PROBLEMS_QUERY_KEY] });
    },
  });
