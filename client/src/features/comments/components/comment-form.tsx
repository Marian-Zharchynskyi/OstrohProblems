...
export function CommentForm({ open, onOpenChange, onSubmit, initialData, isLoading, problemId }: CommentFormProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-xl w-full [&>button]:focus:outline-none [&>button]:focus:ring-0 [&>button]:focus-visible:ring-0 [&>button]:focus-visible:ring-offset-0 [&>button]:border-0">
        <DialogHeader>
          <DialogTitle>{initialData ? 'Редагувати коментар' : 'Створити коментар'}</DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit}>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              {isLoading ? (initialData ? 'Оновлення...' : 'Створення...') : initialData ? 'Оновити' : 'Створити'}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
