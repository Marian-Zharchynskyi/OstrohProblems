...
export function SignalRProvider({ children }: { children: ReactNode }) {
  ...
  const isCoordinatorOrAdmin = user?.roles?.some((role) => role === 'Coordinator' || role === 'Administrator');
  ...
  const onCommentReceived = useCallback((callback: (comment: Comment) => void) => {
    signalRService.onCommentReceived(callback);
  }, []);

  const onProblemsUpdated = useCallback((callback: () => void) => {
