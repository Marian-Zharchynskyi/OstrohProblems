...
const CATEGORY_COLORS = ['#E42556', '#3B82F6', '#10B981', '#F59E0B', '#8B5CF6', '#EC4899', '#14B8A6', '#F97316', '#6366F1', '#84CC16'];
...
export function AdminDashboardPage() {
  ...
  const monthlyChange = stats.problemsLastMonth > 0 ? Math.round(((stats.problemsThisMonth - stats.problemsLastMonth) / stats.problemsLastMonth) * 100) : 0;

  // Filter stats based on active filter
  const filteredCategoryStats = activeFilter?.category ? stats.categoryStats.filter((c) => c.category === activeFilter.category) : stats.categoryStats;

  const filteredStatusStats = activeFilter?.status ? stats.statusStats.filter((s) => s.status === activeFilter.status) : stats.statusStats;

  const filteredPriorityStats = activeFilter?.priority ? stats.priorityStats.filter((p) => p.priority === activeFilter.priority) : stats.priorityStats;
    ...
    <div className="space-y-6 pb-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatCard
          ...
          trend={monthlyChange}
        />
        <StatCard icon={<Clock className="w-5 h-5" />} label="Нових" value={stats.newProblems} color="#F59E0B" subtitle={`${stats.inProgressProblems} в роботі`} />
        <StatCard icon={<CheckCircle className="w-5 h-5" />} label="Виконано" value={stats.completedProblems} color="#10B981" subtitle={`${stats.resolutionRate}% вирішення`} />
        <StatCard
              ...
              <Tooltip
                ...
                formatter={
                  ((value: number | string | undefined, name: string | undefined) => [
                    `${value ?? 0} (${filteredStatusStats.find((s) => s.status === name)?.percentage ?? 0}%)`,
                    name ?? '',
                  ]) as never
                }
              />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 hover:shadow-md transition-shadow">
          ...
          <ResponsiveContainer width="100%" height={280}>
            <PieChart>
              <Pie data={filteredPriorityStats} dataKey="count" nameKey="priority" cx="50%" cy="50%" outerRadius={110} animationBegin={200} animationDuration={800}>
              ...
              <Tooltip
                ...
                formatter={
                  ((value: number | string | undefined, name: string | undefined) => [
                    `${value ?? 0} (${filteredPriorityStats.find((p) => p.priority === name)?.percentage ?? 0}%)`,
                    name ?? '',
                  ]) as never
                }
              />
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 hover:shadow-md transition-shadow">
          ...
          <div className="space-y-3">
            {stats.recentProblems.map((problem) => (
              <Link key={problem.id} to={`/problems/${problem.id}`} className="block p-3 rounded-xl bg-gray-50 hover:bg-gray-100 transition-colors group">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    <p className="font-medium text-gray-900 truncate group-hover:text-[#E42556] transition-colors">{problem.title}</p>
                    <div className="flex items-center gap-2 mt-1 flex-wrap">
                      <span className="text-xs px-2 py-0.5 rounded-full text-white font-medium" style={{ backgroundColor: STATUS_COLORS[problem.status] || '#94a3b8' }}>
                        {problem.status}
                      </span>
                      <span className="text-xs px-2 py-0.5 rounded-full text-white font-medium" style={{ backgroundColor: PRIORITY_COLORS[problem.priority] || '#94a3b8' }}>
                        {problem.priority}
                      </span>
                      <span className="text-xs text-gray-400">{new Date(problem.createdAt).toLocaleDateString('uk-UA')}</span>
                    </div>
                  </div>
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-xs font-medium text-gray-600">{problem.averageRating?.toFixed(1) || 'N/A'}</span>
                      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden hover:shadow-md transition-shadow">
        ...
        {/* Chat Messages */}
        <div className={`overflow-y-auto transition-all duration-300 ${isChatExpanded ? 'max-h-[500px] min-h-[200px]' : 'max-h-[200px]'}`}>
            ...
            <div className="p-4 space-y-4">
              {chatMessages.map((msg) => (
                <div key={msg.id} className={`flex gap-3 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  ...
                  <div className={`max-w-[80%] rounded-2xl px-4 py-3 ${msg.role === 'user' ? 'bg-[#1F2732] text-white rounded-br-md' : 'bg-gray-100 text-gray-800 rounded-bl-md'}`}>
                    {msg.role === 'assistant' ? (
...
// Stat Card Component
function StatCard({ icon, label, value, color, subtitle, trend }: { icon: React.ReactNode; label: string; value: number; color: string; subtitle?: string; trend?: number }) {
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 hover:shadow-md transition-all hover:-translate-y-0.5 group">
      <div className="flex items-center justify-between mb-3">
        <div className="w-10 h-10 rounded-xl flex items-center justify-center transition-transform group-hover:scale-110" style={{ backgroundColor: `${color}15`, color }}>
          {icon}
        </div>
...
// Mini Stat Card
function MiniStatCard({ icon, label, value, suffix = '' }: { icon: React.ReactNode; label: string; value: number; suffix?: string }) {
  return (
    <div className="bg-gradient-to-r from-gray-50 to-white rounded-xl border border-gray-100 p-4 flex items-center gap-3 hover:shadow-sm transition-all">
      ...
      <div>
        <p className="text-lg font-bold text-gray-900">
          {typeof value === 'number' && Number.isInteger(value) ? <AnimatedCounter end={value} /> : value}
          {suffix}
        </p>
