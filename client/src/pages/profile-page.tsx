...
export function ProfilePage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const updated = await userService.updateUser(userDetails.id, formData, token);
      ...
      if (errorMessage.includes('OAuth') || errorMessage.includes('linked to OAuth provider')) {
        toast.error(
          "Email пов'язаний з OAuth провайдером (Google, Facebook) і не може бути змінений через додаток. Оновіть його через налаштування вашого облікового запису провайдера."
        );
      ...
      if (passwordData.currentPassword) {
        try {
          ...
          if (axiosError.response?.status === 400 || axiosError.response?.status === 404) {
            ...
            await userService.setPassword(userDetails.id, { newPassword: passwordData.newPassword }, token);
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const updated = await userService.uploadUserImage(userDetails.id, file, token);
      setUserDetails(updated);
  ...
  const getTabStyles = (isActive: boolean): CSSProperties => ({
    backgroundColor: isActive ? designSystem.colors.profile.tabs.activeBackground : designSystem.colors.profile.tabs.background,
    color: isActive ? designSystem.colors.profile.tabs.text : designSystem.colors.profile.tabs.inactiveText,
    borderColor: isActive ? designSystem.colors.profile.tabs.activeBorder : designSystem.colors.profile.tabs.border,
    boxShadow: 'none',
  });
  ...
  return (
    <div>
      {/* Верхня панель */}
      <div className="py-2" style={{ backgroundColor: designSystem.colors.profile.headerBackground }}>
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
          ...
          <div className="flex flex-col md:flex-row items-center justify-center gap-6 text-center md:text-left md:items-center">
            <div className="relative">
              <div className="w-24 h-24 rounded-full bg-gray-200 flex items-center justify-center overflow-hidden">
                {userDetails.image?.url ? <img src={userDetails.image.url} alt="Profile" className="w-full h-full object-cover" /> : <User className="w-12 h-12 text-gray-500" />}
              </div>
            ...
            <div className="flex flex-col items-center md:items-start">
              <h2 className="text-xl font-bold text-[#1F2732] font-['Mulish'] mb-2">Редагування профілю</h2>
              <p className="text-[#464646] font-['Mulish']">Змініть деталі за вашими уподобаннями</p>
            </div>
          </div>
        </div>
        ...
        <Card className="bg-white border border-gray-200 mb-6 rounded-[10px]">
          <CardHeader className="flex flex-row items-center justify-between pb-8">
            <CardTitle className="text-lg font-bold text-[#1F2732] font-['Mulish']">Основна інформація</CardTitle>
            <button
              type="button"
              onClick={() => {
          ...
          <CardContent>
            ...
            <div className="flex justify-between items-center mt-6">
              ...
              {isBasicInfoEditing && (
                <Button onClick={handleSave} disabled={isSaving} className="bg-[#E42556] hover:bg-[#E42556]/90 text-white rounded-lg px-6">
        ...
        <Card className="bg-white border border-gray-200 mb-6 rounded-[10px]">
          <CardHeader className="flex flex-row items-center justify-between pb-8">
            <CardTitle className="text-lg font-bold text-[#1F2732] font-['Mulish']">Безпека профілю</CardTitle>
            <button
              type="button"
              onClick={() => {
          ...
          <CardContent>
            ...
            {passwordError && <div className="mb-4 p-3 bg-red-50 border border-red-200 text-red-700 rounded-lg text-sm">{passwordError}</div>}
            {passwordSuccess && <div className="mb-4 p-3 bg-green-50 border border-green-200 text-green-700 rounded-lg text-sm">{passwordSuccess}</div>}

            <div className="flex justify-between items-center mt-6">
              <p className="text-sm font-extrabold text-[#1F2732] font-['Mulish']">
                {hasPassword ? 'Введіть існуючий пароль, щоб створити новий' : 'Створіть пароль для можливості входу через email'}
              </p>
              {isSecurityEditing && (
                <Button onClick={handlePasswordChange} disabled={isSaving} className="bg-[#E42556] hover:bg-[#E42556]/90 text-white rounded-lg px-6">
                  {isSaving ? 'Збереження...' : 'Зберегти'}
                </Button>
              )}
            </div>
        ...
        <Card className="bg-white border border-gray-200 rounded-[10px]">
          <CardHeader className="pb-8">
            <CardTitle className="text-lg font-bold text-[#1F2732] font-['Mulish']">Видалити профіль</CardTitle>
          </CardHeader>
          <CardContent>
