...
export function CreateIssueAiPage() {
  ...
  const inputBaseClasses =
    'w-full bg-[#F5F6F7] border-none rounded-[10px] px-4 text-sm focus:ring-0 transition-all outline-none placeholder:text-gray-400'
    ...
    return (
      ...
      >
        ...
        >
          ...
          <div
            className={cn('text-xs mt-1', isUser ? 'text-white/70' : 'text-gray-500')}
          >
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          ...
          <p className="text-gray-500 mt-1">
            Опишіть проблему голосом або текстом — AI заповнить форму автоматично
          </p>
        </div>
      </div>
        ...
        <Card className="bg-white border shadow-sm rounded-xl overflow-hidden flex flex-col h-[700px]">
          <CardHeader className="border-b border-gray-100 bg-gradient-to-r from-violet-500 to-purple-600 text-white py-4">
            ...
            <p className="text-sm text-white/80">
              Опишіть проблему, і я заповню форму
            </p>
          </CardHeader>
          ...
          {isRecording && (
            <div className="px-4 py-2 bg-red-50 border-t flex items-center justify-between">
              ...
              <span className="text-xs text-gray-500">Натисніть для зупинки</span>
            </div>
          )}
          ...
          <div className="p-4 border-t bg-gray-50">
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                className="flex-1 px-4 py-3 bg-white rounded-full text-sm border border-gray-200 focus:outline-none focus:border-violet-400 disabled:opacity-50"
              />
              <button
                onClick={handleSendMessage}
                disabled={isLoading || isProcessingVoice || !inputValue.trim()}
                className="p-3 bg-violet-500 text-white rounded-full hover:bg-violet-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed focus:outline-none"
            ...
            <CardHeader className="border-b border-gray-100 bg-white pb-4 flex flex-row items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-500">
                  {extractedData
                    ? 'Перевірте та відредагуйте за потреби'
                    : 'Дані з\'являться після опису проблеми'}
                </p>
              </div>
              {extractedData && (
                <Button
                ...
                <>
                  <div className="flex flex-col gap-2">
                    ...
                    {isEditing ? (
                      <input
                        ...
                        className={`${inputBaseClasses} h-10`}
                      />
                    ) : (
                      <p className="text-gray-700 bg-gray-50 rounded-lg px-4 py-2">
                        {formData.title || '-'}
                      </p>
                    )}
                  </div>
                      ...
                      {isEditing ? (
                        <div className="relative">
                          <select
                            ...
                            className={`${inputBaseClasses} h-10 appearance-none cursor-pointer`}
                          >
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    {isEditing ? (
                      ...
                      <p className="text-gray-700 bg-gray-50 rounded-lg px-4 py-2 whitespace-pre-wrap">
                  ...
                  {extractedData.streetName && (
                    <div className="flex flex-col gap-2">
                      <Label className="text-[#1F2732] font-medium">
                        Розпізнана вулиця
                      </Label>
                      <p className="text-violet-600 bg-violet-50 rounded-lg px-4 py-2">
                        {extractedData.streetName}
                      </p>
                    </div>
                  )}
                ...
                <CardContent className="p-6">
                  <div className="grid grid-cols-4 gap-3">
                    ...
                    {[...Array(3)].map((_, i) => (
                      <div
                        key={i}
                        className="aspect-square bg-[#F5F6F7] rounded-lg flex items-center justify-center text-gray-300"
                      >
                ...
                <CardContent className="p-6">
                  ...
                  <div className="flex items-center justify-between mt-4">
                    <div className="text-sm">
                      <span className="text-[#1F2732] font-medium">Вулиця:</span>{' '}
                      <span className="text-gray-500">{streetName || 'Не вибрано'}</span>
                    </div>
