...
Система активно використовує передові можливості великих мовних моделей (LLM) для забезпечення зручного та інноваційного користувацького досвіду, без використання складних векторних баз даних (Embeddings/RAG).

### Основні AI-функції:
1.  **Голосовий асистент подачі звернень**:
    *   Користувачі можуть записувати голос прямо в браузері (формат WebM).
    *   Аудіо кодується в Base64 та відправляється до Gemini API (мультимодальна обробка).
    *   Нейромережева модель виконує розпізнавання мови та одночасну екстракцію сутностей (Named Entity Recognition).
    *   Використовується жорстка структурована генерація (Structured JSON) для отримання готових полів проблеми: локація, опис, заголовок, категорія.
2.  **Інтелектуальний чат-бот (Zero-Shot Classification)**:
    *   Забезпечує класифікацію намірів користувача (Intent Classification) виключно за допомогою просунутого Prompt Engineering.
    *   Допомагає з навігацією, правилами подачі проблем та надає консультації.
3.  **Безпека та форматування**:
    *   Вбудований фільтр ненормативної лексики (Profanity Filter) для блокування образливого контенту під час спілкування з ШІ.
    *   Автоматичне форматування відповідей ШІ з розбиттям на зручні абзаци для покращення читабельності.
...
Система повністю готова до промислового використання (Production-ready) завдяки використанню індустріальних практик DevOps, контейнеризації та підходу «Інфраструктура як код» (IaC).

### Архітектура розгортання:
*   **Хмарний провайдер**: Amazon Web Services (AWS).
*   **Terraform**: Вся інфраструктура описана декларативно в директорії `terraform/`. Це включає налаштування VPC, публічних/приватних підмереж, Security Groups та маршрутизації.
*   **Контейнеризація (Docker)**: Backend та Frontend ізольовані в контейнерах, образи яких зберігаються в приватному реєстрі Amazon ECR.
*   **Безсерверні обчислення (AWS Fargate)**: Кластер Amazon ECS запускає контейнери на базі AWS Fargate, що позбавляє від необхідності ручного управління серверами EC2.
*   **Балансування навантаження (ALB)**: Application Load Balancer розподіляє вхідний HTTP/HTTPS трафік між екземплярами додатків.
*   **База Даних**: Керований сервіс Amazon RDS (PostgreSQL) розгорнутий у приватній підмережі для максимальної безпеки даних.

### CI/CD Пайплайни (GitHub Actions):
Проект має налаштовані пайплайни безперервної інтеграції та доставки (`deploy.yml`). При злитті коду в гілку `main`:
1.  Автоматично збираються нові Docker-образи клієнта та сервера.
2.  Образи публікуються в AWS ECR.
3.  ECS кластер виконує плавне оновлення (Rolling Update), поступово замінюючи старі контейнери новими, що гарантує **Zero-Downtime Deployment** (оновлення без простою для користувачів).
